      ...
      >
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              href="#pricing"
              className="rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-indigo-600"
            >
              Start Free Trial
            </a>
          </div>
      ...
      <main id="top">
        <section className="mx-auto grid max-w-7xl gap-12 px-6 pb-16 pt-20 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-slate-600">
              Upload bulk payslips — our AI extracts Employee name, Gross salary,
              Net pay, PF, TDS and 15+ fields automatically. No templates. No
              manual entry.
            </p>
            <div className="mt-8 flex flex-wrap items-center gap-4">
              <a
                href="#pricing"
                className="rounded-lg bg-brand px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-indigo-600"
              >
                Start Free Trial
              </a>
              <a
                href="#how-it-works"
                className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-100"
              >
                See How It Works
            ...
            <div className="grid gap-4 md:grid-cols-[1fr_auto_1fr] md:items-center">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="space-y-2 text-xs text-slate-600">
                  <p className="rounded bg-white px-3 py-2">
                    Employee Payslip - Jan.pdf
                  </p>
                  <p className="rounded bg-white px-3 py-2">
                    Employee Payslip - Feb.pdf
                  </p>
                  <p className="rounded bg-white px-3 py-2">
                    Employee Payslip - Mar.pdf
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="space-y-1 text-xs text-slate-700">
                  <p className="grid grid-cols-2 gap-2 rounded bg-white px-3 py-2">
                    <span>Name</span>
                    <span>Net Pay</span>
                  </p>
                  <p className="grid grid-cols-2 gap-2 rounded bg-white px-3 py-2">
        ...
        <section className="border-y border-slate-200 bg-white py-8">
          <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-5 px-6 md:flex-row">
            <p className="text-center text-sm font-medium text-slate-700 md:text-left">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs font-semibold uppercase tracking-wide text-slate-500">
              {['CA Firm', 'NBFC', 'HR Tech'].map((logo) => (
                <div
                  key={logo}
                  className="rounded-md border border-slate-200 bg-slate-50 px-4 py-2 text-center"
                >
                  {logo}
                </div>
              ))}
        ...
        <section id="features" className="bg-white py-20">
          <div className="mx-auto max-w-7xl px-6">
            <h2 className="text-center text-3xl font-bold tracking-tight">
              Features Built for Indian Businesses
            </h2>
        ...
        <section id="pricing" className="mx-auto max-w-7xl px-6 py-20">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {plans.map((plan) => (
              <article
                key={plan.name}
                className={`flex rounded-xl border bg-white p-6 ${
                  plan.popular
                    ? 'border-brand shadow-md ring-2 ring-indigo-100'
                    : 'border-slate-200'
                }`}
              >
                <div className="flex w-full flex-col">
                  {plan.popular && (
                    <span className="mb-4 inline-flex w-fit rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-brand">
                      Most Popular
                    </span>
                  )}
        ...
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-6 text-sm text-slate-600 md:flex-row">
          <div className="flex items-center gap-2 font-semibold text-slate-900">
            ...
            <span className="font-normal text-slate-500">
              AI-powered payslip to Excel converter
            </span>
          </div>
